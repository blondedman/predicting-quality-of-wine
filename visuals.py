...
def distribution(data, feature_label, transformed = False):
    ...
    for i, feature in enumerate([feature_label]):
        ...
        ax.hist(data[feature], bins = 20, color = '#2a8edb')
        ax.set_title("'%s' Feature Distribution"%(feature), fontsize = 10)
...
def feature_plot(importances, X_train, y_train):
    ...
    # plt.bar(np.arange(11) - 0.3, np.cumsum(values), width = 0.2, align = "center", color = '#00A0A0',label = "Cumulative Feature Weight")
